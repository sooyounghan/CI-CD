...
5. AWS
   ...
   - AWS ECR : Docker 이미지를 쉽게 저장 및 관리 가능하며, 관리형 컨테이너 레지스트리 서비스이며, 이를 사용하면 생성한 Docker 이미지를 안전하게 저장하고 필요할 때, Kubernetes와 같은 서비스에 쉽게 배포 가능
   - AWS EKS : AWS에서 제공하는 관리형 Kubernetes 서비스로, Kubernetes 클러스터를 손쉽게 설정하고, 고가용성 / 신뢰성 / 확장성 등을 클라우드 환경에서 제공 
